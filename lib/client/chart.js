...
function init (client, d3, $) {
  ...
  chart.xScaleBasals = d3.time.scale()
    .domain(extent);
  ...
  chart.futureOpacity = d3.scale.linear()
    .domain([times.mins(25)
      .msecs, times.mins(60)
      .msecs])
    .range([0.8, 0.1]);

  // create svg and g to contain the chart contents
  chart.charts = d3.select('#chartContainer')
    .append('svg')
    .append('g')
  ...
  chart.drag = chart.focus.append('g')
  ...
  chart.update = function update (init) {
    ...
    if (widthChanged || (chart.prevChartHeight !== chartHeight)) {
      ...
      if (init) {
        ...
        d3.select('.brush')
          .call(chart.brush.clear());

        // redraw old brush with new dimensions
        d3.select('.brush')
          .transition()
          .call(chart.brush.extent(currentBrushExtent));
    ...
    var updateBrush = d3.select('.brush')
      .transition();
    updateBrush
  ...
  chart.scroll = function scroll (nowDate) {
    ...
    // update x,y axis
    chart.focus.select('.x.axis')
      .call(chart.xAxis);
    chart.focus.select('.y.axis')
      .call(chart.yAxis);
