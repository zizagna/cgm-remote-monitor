...
function init (client, $) {
    ...
    if ($('#bc_othertime')
      ...
      if (mergeDateAndTime() < moment()) {
        $('#bc_retro')
          ...
          .text(translate('RETRO MODE'));
      } else if (mergeDateAndTime() > moment()) {
        $('#bc_retro')
          .css('background-color', 'blue')
          .text(translate('IN THE FUTURE'));
      } else {
        $('#bc_retro')
          .css('display', 'none');
      }
  ...
  boluscalc.prepare = function prepare () {
    ...
    profiles.forEach(function(p) {
      $('#bc_profile')
        .append('<option val="' + p + '">' + p + '</option>');
    });
    $('#bc_profileLabel')
      .toggle(isProfileEnabled(profiles));
    ...
    $('#bc_notes')
      .val('');
    $('#bc_enteredBy')
      .val(Storages.localStorage.get('enteredBy') || '');
    $('#bc_nowtime')
      .prop('checked', true);
    $('#bc_othercorrection')
      .val(0);
    $('#bc_profile')
      .val(client.profilefunctions.activeProfileToTime());
    setDateAndTime();
    boluscalc.eventTimeTypeChange();
    boluscalc.updateVisualisations(client.sbx);
    boluscalc.calculateInsulin();
  ...
  boluscalc.updateGui = function updateGui (record) {
    ...
    if ($('#bc_useiob')
      .is(':checked')) {
      $('#bc_iob')
        .text((record.iob > 0 ? '-' : '') + record.iob.toFixed(2));
    } else {
      $('#bc_iob')
        .text('');
    }
    ...
    if ($('#bc_usecob')
      .is(':checked')) {
      $('#bc_cob')
        .text(record.cob.toFixed(2));
      $('#bc_cobu')
        .text(record.insulincob.toFixed(2));
    } else {
      $('#bc_cob')
        .text('');
      $('#bc_cobu')
    ...
    if ($('#bc_usebg')
      .is(':checked')) {
      if (record.bg === 0 || (boluscalc.oldbg && $('#bc_bgfromsensor')
          .is(':checked'))) {
        $('#bc_bg')
          .css('background-color', 'red');
      } else {
      ...
      $('#bc_inzulinbg')
        .text(record.insulinbg.toFixed(2));
      $('#bc_inzulinbg')
        .attr('title'
    ...
    if (record.foods.length) {
      ...
      for (var fi = 0; fi < record.foods.length; fi++) {
        ...
        html += '<td>';
        if ($('#bc_quickpick')
          .val() < 0) { // do not allow deleting while quickpick active
    ...
    if ($('#bc_usecarbs')
      ...
      if ($('#bc_carbs')
        ...
        $('#bc_carbs')
          .css('background-color', '');
      } else if (isNaN(parseInt($('#bc_carbs')
          .val()
          .replace(',', '.')))) {
        $('#bc_carbs')
          .css('background-color', 'red');
      ...
      $('#bc_inzulincarbs')
        .text(record.insulincarbs.toFixed(2));
      $('#bc_inzulincarbs')
        .attr('title', 'IC: ' + ic);
    } else {
      $('#bc_carbs')
    ...
    // Show Total
    $('#bc_rouding')
      .text(record.roundingcorrection.toFixed(2));
    $('#bc_insulintotal')
      .text(record.insulin.toFixed(2));
    ...
    if (record.othercorrection === 0 && record.carbs === 0 && record.cob === 0 && record.bg > 0 && outcome > targetBGLow && outcome < targetBGHigh) {
      ...
      $('#bc_insulinneededtr')
        .css('display', 'none');
      $('#bc_calculationintarget')
        .css('display', '');
    } else if (record.insulin < 0) {
      $('#bc_carbsneeded')
        .text(record.carbsneeded + ' g');
      $('#bc_insulinover')
        .text(record.insulin.toFixed(2));
      $('#bc_carbsneededtr')
        .css('display', '');
    ...
    tempMark += tempMark ? ': ' : '';
    $('#bc_basal')
      .text(tempMark + basal.totalbasal.toFixed(3));
  };
  ...
  boluscalc.gatherBoluscalcData = function gatherBoluscalcData () {
    ...
    if ($('#bc_othertime')
      .is(':checked')) {
      record.eventTime = mergeDateAndTime()
        .toDate();
    }
    ...
    if ($('#bc_useiob')
      .is(':checked')) {
      record.iob = roundTo(iob.calcTotal(client.sbx.data.treatments, client.sbx.data.devicestatus, client.sbx.data.profile, record.eventTime, record.profile)
        .iob, 0.01);
    }
    ...
    if ($('#bc_usecob')
      .is(':checked')) {
      record.cob = roundTo(cob.cobTotal(client.sbx.data.treatments, client.sbx.data.devicestatus, client.sbx.data.profile, record.eventTime, record.profile)
        .cob, 0.01);
    ...
    if ($('#bc_usecarbs')
      ...
      if (record.carbs === 0) { // not set from foods
        record.carbs = parseInt($('#bc_carbs')
          .val()
          .replace(',', '.'));
      }
    ...
    // Corrections
    record.othercorrection = parseFloat($('#bc_othercorrection')
      .val());
  ...
  function gatherData () {
    ...
    if ($('#bc_bg')
      ...
      data.glucose = $('#bc_bg')
        .val()
        .replace(',', '.');
      data.glucoseType = $('#boluscalc-form')
        .find('input[name=bc_bginput]:checked')
        .val();
    ...
    data.carbs = $('#bc_carbs')
      .val()
      .replace(',', '.');
    data.insulin = $('#bc_insulintotal')
      .text();
    if (data.insulin <= 0) {
      delete data.insulin;
    }
    data.preBolus = parseInt($('#bc_preBolus')
      .val());
    data.notes = $('#bc_notes')
      .val();
  ...
  function buildConfirmText (data) {
    ...
    text.push(translate('Event Time') + ': ' + (data.eventTime ? data.eventTime.toLocaleString() : new Date()
      .toLocaleString()));
    return text.join('\n');
  }
  ...
  boluscalc.loadFoodQuickpicks = function loadFoodQuickpicks () {
    ...
    $('#bc_quickpick')
      .empty()
      .append('<option value="-1">' + translate('(none)') + '</option>');
    for (var i = 0; i < records.length; i++) {
      var r = records[i];
      $('#bc_quickpick')
        .append('<option value="' + i + '">' + r.name + ' (' + r.carbs + ' g)</option>');
    };
    $('#bc_quickpick')
      .val(-1);
    $('#bc_quickpick')
      .change(quickpickChange);
  };
  ...
  function fillForm (event) {
    ...
    Object.keys(categories)
      .forEach(function eachCategory (s) {
        $('#bc_filter_category')
  ...
  function fillSubcategories (event) {
    maybePrevent(event);
    filter.category = $('#bc_filter_category')
      .val();
    filter.subcategory = '';
    $('#bc_filter_subcategory')
      .empty()
    ...
    if (filter.category !== '') {
      Object.keys(categories[filter.category])
        .forEach(function eachSubcategory (s) {
          $('#bc_filter_subcategory')
            .append('<option value="' + s + '">' + s + '</option>');
        });
    }
  ...
  function doFilter (event) {
    if (event) {
      filter.category = $('#bc_filter_category')
        .val();
      filter.subcategory = $('#bc_filter_subcategory')
        .val();
      filter.name = $('#bc_filter_name')
        .val();
    }
    $('#bc_data')
      .empty();
    for (var i = 0; i < foodlist.length; i++) {
      ...
      if (filter.name !== '' && foodlist[i].name.toLowerCase()
        .indexOf(filter.name.toLowerCase()) < 0) { continue; }
      var o = '';
      o += foodlist[i].name + ' | ';
      ...
      $('#bc_data')
        .append('<option value="' + i + '">' + o + '</option>');
    }
    $('#bc_addportions')
      .val('1');
    ...
    $('#bc_addfooddialog')
      .dialog({
        ...
        , buttons: [
          {
            ...
            , click: function() {
              var index = $('#bc_data')
                .val();
              var portions = parseFloat($('#bc_addportions')
                .val()
                .replace(',', '.'));
        ...
        , open: function() {
          ...
          $(this)
            .parent()
            .find('button:contains("' + translate('Add') + '")')
            .css({ 'float': 'left' });
          $('#bc_filter_name')
            .focus();
        }
  ...
  function findClosestSGVToPastTime (time) {
    ...
    if (!focusPoint || focusPoint.mills + times.mins(10)
      .mills < time.getTime()) {
      return null;
    }
    return focusPoint;
  ...
  if (isTouch()) {
    ...
    $('.insulincalculationpart')
      .change(boluscalc.calculateInsulin);
  } else {
    $('.insulincalculationpart')
      .on('input', boluscalc.calculateInsulin);
    $('input:checkbox.insulincalculationpart')
      .change(boluscalc.calculateInsulin);
  }
  $('#bc_bgfrommeter')
    .change(boluscalc.calculateInsulin);
  $('#bc_addfromdatabase')
    .click(addFoodFromDatabase);
  $('#bc_bgfromsensor')
    .change(function bc_bgfromsensor_click (event) {
      boluscalc.updateVisualisations(client.sbx);
      boluscalc.calculateInsulin();
      maybePrevent(event);
    });

  $('#boluscalcDrawerToggle')
    .click(boluscalc.toggleDrawer);
  $('#boluscalcDrawer')
    .find('button')
    .click(boluscalc.submit);
  $('#bc_eventTime input:radio')
    .change(boluscalc.eventTimeTypeChange);

  $('.bc_eventtimeinput')
    .focus(boluscalc.dateTimeFocus)
    .change(boluscalc.dateTimeChange);
