...
var DEFAULT_FOCUS = times.hours(3)
  .msecs
...
function init (client, d3) {
  ...
  var dotRadius = function(type) {
    var radius = chart()
      .prevChartWidth > WIDTH_BIG_DOTS ? 4 : (chart()
        .prevChartWidth < WIDTH_SMALL_DOTS ? 2 : 3);
    if (type === 'mbg') {
      radius *= 2;
  ...
  function tooltipLeft () {
    var windowWidth = $(client.tooltip)
      .parent()
      .parent()
      .width();
  ...
  renderer.highlightBrushPoints = function highlightBrushPoints (data) {
    if (data.mills >= chart()
      ...
      .brush.extent()[1].getTime()) {
      return chart()
        .futureOpacity(data.mills - client.latestSGV.mills);
    } else {
      return 0.5;
    }
  };
  ...
  renderer.bubbleScale = function bubbleScale () {
    ...
    return (chart()
      .prevChartWidth < WIDTH_SMALL_DOTS ? 4 : (chart()
        .prevChartWidth < WIDTH_BIG_DOTS ? 3 : 2)) * focusRangeAdjustment();
  };
  ...
  renderer.addFocusCircles = function addFocusCircles () {
    ...
    if (client.sbx.pluginBase.forecastPoints) {
      ...
      var focusHoursAheadMills = chart()
        .brush.extent()[1].getTime() + client.focusRangeMS;
    ...
    var focusCircles = chart()
      .focus.selectAll('circle')
      .data(focusData, client.entryToDate);
    ...
    function prepareFocusCircles (sel) {
      ...
      sel.attr('cx', function(d) {
        ...
        .attr('opacity', function(d) {
          return d.noFade ? 100 : chart()
            .futureOpacity(d.mills - client.latestSGV.mills);
        })
    ...
    prepareFocusCircles(focusCircles.enter()
        .append('circle'))
  ...
  renderer.addTreatmentCircles = function addTreatmentCircles () {
    ...
    // bind up the focus chart data to an array of circles
    var treatCircles = chart()
      .focus.selectAll('treatment-dot')
      .data(client.ddata.treatments.filter(function(treatment) {
    ...
    function prepareTreatCircles (sel) {
      ...
      sel.attr('cx', function(d) {
        ...
        .attr('cy', function(d) {
          return chart()
            .yScale(client.sbx.scaleEntry(d));
        })
        .attr('r', function() {
    ...
    // if new circle then just display
    prepareTreatCircles(treatCircles.enter()
        .append('circle'))
    ...
    var treatRects = chart()
      .focus.selectAll('.g-duration')
    ...
    function rectTranslate (d) {
      ...
      return 'translate(' + chart()
        .xScale(new Date(d.mills)) + ',' + chart()
        .yScale(utils.scaleMgdl(top)) + ')';
    }
    ...
    chart()
      .focus.selectAll('.g-duration-rect')
      .transition()
      .attr('width', function(d) {
        return chart()
          .xScale(new Date(d.mills + times.mins(d.duration)
            .msecs)) - chart()
          .xScale(new Date(d.mills));
      });

    chart()
      .focus.selectAll('.g-duration-text')
      .transition()
      .attr('transform', function(d) {
        return 'translate(' + (chart()
          .xScale(new Date(d.mills + times.mins(d.duration)
            .msecs)) - chart()
          .xScale(new Date(d.mills))) / 2 + ',' + 10 + ')';
      });

    // if new rect then just display
    var gs = treatRects.enter()
      .append('g')
  ...
  renderer.addContextCircles = function addContextCircles () {
    ...
    // if new circle then just display
    prepareContextCircles(contextCircles.enter()
      .append('circle'));
  ...
  function appendTreatments (treatment, arc) {
    ...
    function boluscalcTooltip (treatment) {
      ...
      var html = '<hr>';
      html += (treatment.boluscalc.othercorrection ? '<strong>' + translate('Other correction') + ':</strong> ' + parseFloat(treatment.boluscalc.othercorrection)
        .toFixed(2) + 'U<br/>' : '');
      ...
      if (treatment.boluscalc.foods && treatment.boluscalc.foods.length) {
        ...
        for (var fi = 0; fi < treatment.boluscalc.foods.length; fi++) {
          ...
          html += '<tr>';
          html += '<td>' + f.name + '</td>';
          html += '<td>' + (f.portion * f.portions)
            .toFixed(1) + ' ' + f.unit + '</td>';
          html += '<td>(' + (f.carbs * f.portions)
            .toFixed(1) + ' g)</td>';
          html += '</tr>';
        }
        html += '</table>';
    ...
    renderer.drag = d3.behavior.drag()
      ...
      .on('drag', function() {
        ...
        client.tooltip.transition()
          .style('opacity', .9);
        var x = Math.min(Math.max(0, d3.event.x), chart()
          .charts.attr('width'));
        var y = Math.min(Math.max(0, d3.event.y), chart()
          .focusHeight);
        ...
        newTime = new Date(chart()
          .xScale.invert(x));
        var minDiff = times.msecs(newTime.getTime() - treatment.mills)
          .mins.toFixed(0);
        client.tooltip.html(
        ...
        chart()
          ...
          .attr({
            ...
            , 'x1': chart()
              .xScale(new Date(treatment.mills))
            , 'y1': chart()
              .yScale(client.sbx.scaleEntry(treatment))
            , 'x2': x
            , 'y2': y
            , 'stroke-width': 2
            , 'stroke': 'white'
    ...
    var treatmentDots = chart()
      .focus.selectAll('treatment-insulincarbs')
      .data(arc.data)
      .enter()
      .append('g')
      .attr('class', 'draggable-treatment')
      .attr('transform', 'translate(' + chart()
        .xScale(new Date(treatment.mills)) + ', ' + chart()
        .yScale(client.sbx.scaleEntry(treatment)) + ')')
  ...
  renderer.drawTreatments = function drawTreatments (client) {
    ...
    chart()
      .focus.selectAll('.draggable-treatment')
      .remove();
  ...
  renderer.drawTreatment = function drawTreatment (treatment, opts, carbratio) {
    ...
    if (Math.abs(chart()
        .xScale(new Date(treatment.mills))) > innerWidth) {
      return;
    }
  ...
  renderer.addBasals = function addBasals (client) {
    ...
    var comboareadata = [];
    var from = chart()
      .brush.extent()[0].getTime();
    var to = Math.max(chart()
      .brush.extent()[1].getTime(), client.sbx.time) + client.forecastTime;
    ...
    chart()
      .maxBasalValue = max;
    chart()
      .yScaleBasals.domain('icicle' === mode ? [0, max] : [max, 0]);

    chart()
      .basals.selectAll('g')
      .remove();
    chart()
      .basals.selectAll('.basalline')
      .remove()
      .data(linedata);
    chart()
      .basals.selectAll('.notempline')
      .remove()
      .data(notemplinedata);
    chart()
      .basals.selectAll('.basalarea')
      .remove()
      .data(basalareadata);
    chart()
      .basals.selectAll('.tempbasalarea')
      .remove()
      .data(tempbasalareadata);
    chart()
      .basals.selectAll('.comboarea')
      .remove()
      .data(comboareadata);
    ...
    _.forEach(client.ddata.tempbasalTreatments, function eachTemp (t) {
      ...
      if (t.duration && t.mills < to && t.mills + times.mins(t.duration)
        .msecs > from) {
        var text = g.append('text')
          ...
          .attr('dy', '.35em')
          .attr('x', chart()
            .xScaleBasals((Math.max(t.mills, from) + Math.min(t.mills + times.mins(t.duration)
              .msecs, to)) / 2))
          .attr('y', 10)
          .text((t.percent ? (t.percent > 0 ? '+' : '') + t.percent + '%' : '') + (isNaN(t.absolute) ? '' : Number(t.absolute)
            .toFixed(2) + 'U') + (t.relative ? 'C: +' + t.relative + 'U' : ''));
        // better hide if not fit
        if (text.node()
          .getBBox()
          .width > chart()
          .xScaleBasals(t.mills + times.mins(t.duration)
            .msecs) - chart()
          .xScaleBasals(t.mills)) {
  ...
  renderer.addTreatmentProfiles = function addTreatmentProfiles (client) {
    if (client.profilefunctions.listBasalProfiles()
      .length < 2) {
    ...
    var to = chart()
      .brush.extent()[1].getTime();
    var mult = (to - from) / times.hours(24)
      .msecs;
    from += times.mins(20 * mult)
      .msecs;
    ...
    _.forEach(client.ddata.profileTreatments, function eachTreatment (d) {
      if (d.duration && !d.cuttedby) {
        data.push({
          cutting: d.profile
          , profile: client.profilefunctions.activeProfileToTime(times.mins(d.duration)
            .msecs + d.mills + 1)
          , mills: times.mins(d.duration)
            .msecs + d.mills
          , end: true
        });
      }
    ...
    var treatProfiles = chart()
      .basals.selectAll('.g-profile')
      .data(data);

    var topOfText = ('icicle' === mode ? chart()
      .maxBasalValue + 0.05 : -0.05);
    ...
    treatProfiles.transition()
      ...
      .attr('transform', function(t) {
        ...
        return 'rotate(-90,' + chart()
          .xScale(t.mills) + ',' + chart()
