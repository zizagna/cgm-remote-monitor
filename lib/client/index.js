  ...
  $.ajax({
    ...
    .fail(function fail (jqXHR, textStatus, errorThrown) {
      ...
      if (jqXHR.readyState == 0) {
        ...
        $('#loadingMessageText')
          .html('Connecting to Nightscout server failed, retrying every 2 seconds');
...
client.load = function load (serverSettings, callback) {
  ...
  client.forecastTime = times.mins(30)
    .msecs;
  client.entries = [];
  ...
  client.afterAuth = function afterAuth (isAuthenticated) {
    ...
    $('#lockedToggle')
      ...
      .toggle(!treatmentCreateAllowed && client.settings.showPlugins.indexOf('careportal') > -1);
    $('#treatmentDrawerToggle')
      .toggle(treatmentCreateAllowed && client.settings.showPlugins.indexOf('careportal') > -1);
    $('#boluscalcDrawerToggle')
      .toggle(treatmentCreateAllowed && client.settings.showPlugins.indexOf('boluscalc') > -1);
  ...
  client.focusRangeMS = times.hours(client.settings.focusHours)
    .msecs;
  $('.focus-range li[data-hours=' + client.settings.focusHours + ']')
    .addClass('selected');
  ...
  client.dataExtent = function dataExtent () {
    return client.entries.length > 0 ?
      ...
      d3.extent([new Date(client.now - times.hours(history)
        .msecs), new Date(client.now)]);
  };

  client.bottomOfPills = function bottomOfPills () {
    //the offset's might not exist for some tests
    var bottomOfPrimary = primary.offset() ? primary.offset()
      .top + primary.height() : 0;
    var bottomOfMinorPills = minorPills.offset() ? minorPills.offset()
      .top + minorPills.height() : 0;
    var bottomOfStatusPills = statusPills.offset() ? statusPills.offset()
      .top + statusPills.height() : 0;
    ...
    if (status !== 'current') {
      ...
      if (currentMgdl < 39) {
        ...
        if (delta) {
          ...
          title = s(scaleBg(currentMgdl)) + s(deltaDisplay) + s(client.direction.info(client.latestSGV)
            .label) + title;
        }
      }
    }
  ...
  function checkAnnouncement () {
    var result = {
      inProgress: currentAnnouncement ? Date.now() - currentAnnouncement.received < times.mins(5)
        .msecs : false
    };
  ...
  function updateTitle () {
    ...
    $(document)
      .attr('title', windowTitle || generateTitle());
  }
  ...
  function brushed () {
    ...
    function updatePlugins (time) {
      ...
      client.sbx = sandbox.clientInit(
        client.ctx
        , new Date(time)
        .getTime() //make sure we send a timestamp
    ...
    function updateHeader () {
      ...
      if (focusPoint) {
        if (brushExtent[1].getTime() - focusPoint.mills > times.mins(15)
          .msecs) {
          clearCurrentSGV();
        } else {
  ...
  function generateAlarm (file, notify) {
    ...
    _.each(client.settings.snoozeMinsForAlarmEvent(notify), function eachOption (mins) {
      var snoozeOption = $('<li><a data-snooze-time="' + times.mins(mins)
        .msecs + '">' + client.translate('Silence for %1 minutes', { params: [mins] }) + '</a></li>');
  ...
  function playAlarm (audio) {
    ...
    if (client.browserUtils.queryParms()
      .mute !== 'true') {
      audio.play();
    } else {
  ...
  function stopAlarm (isClient, silenceTime, notify) {
    ...
    silenceTime = silenceTime || times.mins(5)
      .msecs;
  ...
  function refreshAuthIfNeeded () {
    ...
    if (token && client.authorized) {
      var renewTime = (client.authorized.exp * 1000) - times.mins(15)
        .msecs - Math.abs((client.authorized.iat * 1000) - client.authorized.lat);
      ...
      } else if (refreshIn < times.mins(5)
        .secs) {
  ...
  function updateClock () {
    ...
    if (client.settings.nightMode) {
      ...
      if (opacity.current !== opacity.NIGHT && (dateTime.getHours() > 21 || dateTime.getHours() < 7)) {
        $('body')
          .css({ 'opacity': opacity.NIGHT });
      } else {
        $('body')
          .css({ 'opacity': opacity.DAY });
      }
    }
    refreshAuthIfNeeded();
  ...
  $('.focus-range li')
    .click(function(e) {
      ...
      if (li.attr('data-hours')) {
        ...
        client.focusRangeMS = times.hours(hours)
          .msecs;
  ...
  client.resetRetroIfNeeded = function resetRetroIfNeeded () {
    if (client.retro.loadedMills > 0 && Date.now() - client.retro.loadedMills > times.mins(5)
      .msecs) {
      resetRetro();
  ...
  client.loadRetroIfNeeded = function loadRetroIfNeeded () {
    var now = Date.now();
    if (now - client.retro.loadStartedMills < times.secs(30)
      .msecs) {
      console.info('retro already loading, started', new Date(client.retro.loadStartedMills));
      return;
    }

    if (now - client.retro.loadedMills > times.mins(3)
      .msecs) {
  ...
  $('#testAlarms')
    .click(function(event) {
      ...
      d3.selectAll('.audio.alarms audio')
        .each(function() {
          var audio = this;
          playAlarm(audio);
          setTimeout(function() {
  ...
  if (serverSettings) {
    ...
    $('.version')
      .text(serverSettings.version);
    $('.head')
      .text(serverSettings.head);
  ...
  $('.foodcontrol')
    .toggle(client.settings.enable.indexOf('food') > -1);
  // hide cob control if not enabled
  $('.cobcontrol')
    .toggle(client.settings.enable.indexOf('cob') > -1);
  ...
  function prepareEntries () {
    ...
    var tooOld = client.now - times.hours(48)
      .msecs;
