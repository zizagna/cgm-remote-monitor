...
function init (ctx) {
  ...
  loop.analyzeData = function analyzeData (sbx) {
    var recentHours = 6;
    var recentMills = sbx.time - times.hours(recentHours)
      .msecs;
    ...
    var recent = moment(sbx.time)
      .subtract(prefs.warn / 2, 'minutes');
    ...
    function getDisplayForStatus (status) {
      ...
      if (status.failureReason || (status.enacted && !status.enacted.received)) {
        ...
        desc.code = 'error';
        desc.label = 'Error';
      } else if (status.enacted && moment(status.timestamp)
        .isAfter(recent)) {
        desc.symbol = '⌁';
        desc.code = 'enacted';
        desc.label = 'Enacted';
      } else if (status.recommendedTempBasal && moment(status.recommendedTempBasal.timestamp)
        .isAfter(recent)) {
        desc.symbol = '⏀';
      ...
      function toPoints (startTime, offset) {
        return function toPoint (value, index) {
          return {
            mgdl: value
            , color: '#ff00ff'
            , mills: startTime.valueOf() + times.mins(5 * index)
              .msecs + offset
            , noFade: true
          };
        };
  ...
  function alexaForecastHandler (next, slots, sbx) {
    if (sbx.properties.loop.lastLoop.predicted) {
      ...
      var endPrediction = startPrediction.clone()
      ...
      if (endPrediction.valueOf() < sbx.time) {
        ...
        for (var i = 1, len = forecast.slice(0, maxForecastIndex)
            .length; i < len; i++) {
          if (forecast[i] > max) {
            max = forecast[i];
          }
        ...
        var response = 'According to the loop forecast you are expected to be ' + value + ' over the next ' + moment(endPrediction)
          .from(moment(sbx.time));
  ...
  function alexaLastLoopHandler (next, slots, sbx) {
    ...
    var response = 'The last successful loop was ' + moment(sbx.properties.loop.lastOkMoment)
      .from(moment(sbx.time));
  ...
  function statusLevel (prop, prefs, sbx) {
    ...
    if (prop.lastOkMoment) {
      var urgentTime = prop.lastOkMoment.clone()
        .add(prefs.urgent, 'minutes');
      var warningTime = prop.lastOkMoment.clone()
        .add(prefs.warn, 'minutes');
