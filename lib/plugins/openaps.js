...
function init (ctx) {
  ...
  openaps.analyzeData = function analyzeData (sbx) {
    ...
    _.forEach(recentData, function eachStatus (status) {
      ...
      if (moments.iob && (!result.lastIOB || moment(iob.timestamp)
          .isAfter(result.lastIOB.moment))) {
  ...
  openaps.findOfflineMarker = function findOfflineMarker (sbx) {
    return _.findLast(sbx.data.treatments, function match (treatment) {
      ...
      var eventEnd = treatment.duration ? eventTime + times.mins(treatment.duration)
        .msecs : eventTime;
      ...
      function toPoints (offset, forecastType) {
        return function toPoint (value, index) {
          return {
            mgdl: value
            , color: '#ff00ff'
            , mills: prop.lastPredBGs.moment.valueOf() + times.mins(5 * index)
              .msecs + offset
            , noFade: true
  ...
  function alexaLastLoopHandler (next, slots, sbx) {
    ...
    var response = translate('alexaLastLoop', {
      params: [
        moment(sbx.properties.openaps.lastLoopMoment)
  ...
  function statusLevel (prop, prefs, sbx) {
    ...
    if (openaps.findOfflineMarker(sbx)) {
      ...
      var urgentTime = prop.lastLoopMoment.clone()
        .add(prefs.urgent, 'minutes');
      var warningTime = prop.lastLoopMoment.clone()
