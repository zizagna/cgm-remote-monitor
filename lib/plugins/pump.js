...
function init (ctx) {
  ...
  pump.setProperties = function setProperties (sbx) {
    sbx.offerProperty('pump', function setPump () {
      ...
      var recentMills = sbx.time - times.mins(prefs.urgentClock * 2)
        .msecs;
      ...
      _.forEach(filtered, function each (status) {
        status.clockMills = status.pump && status.pump.clock ? moment(status.pump.clock)
          .valueOf() : status.mills;
  ...
  function updateClock (prefs, result, sbx) {
    if (result.clock) {
      ...
      var urgent = moment(sbx.time)
        .subtract(prefs.urgentClock, 'minutes');
      var warn = moment(sbx.time)
        .subtract(prefs.warnClock, 'minutes');
