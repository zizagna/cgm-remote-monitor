...
function init (ctx) {
  ...
  iob.RECENCY_THRESHOLD = times.mins(30)
    .msecs;
  ...
  iob.fromDeviceStatus = function fromDeviceStatus (devicestatusEntry) {
    ...
    if (_.isObject(iobOpenAPS)) {
      ...
      return {
        ...
        , mills: moment(iobOpenAPS.timestamp)
          .valueOf()
      };
      ...
      return {
        ...
        , source: 'Loop'
        , device: devicestatusEntry.device
        , mills: moment(iobLoop.timestamp)
          .valueOf()
      };
  ...
  iob.updateVisualisation = function updateVisualisation (sbx) {
    ...
    if (prop.lastBolus) {
      var when = new Date(prop.lastBolus.mills)
        .toLocaleTimeString();
