...
function init (ctx) {
  ...
  bwp.calc = function calc (sbx) {
    ...
    var recentCarbs = _.findLast(sbx.data.treatments, function eachTreatment (treatment) {
      return treatment.mills <= sbx.time &&
        sbx.time - treatment.mills < times.mins(60)
        .msecs &&
        treatment.carbs > 0;
    });
  ...
  function prepareSettings (sbx) {
    return {
      ...
      , snoozeLength: (sbx.extendedSettings.snoozeMins && Number(sbx.extendedSettings.snoozeMins) * 60 * 1000) || times.mins(10)
        .msecs
    };
  }
      ...
      if (prop.bolusEstimateDisplay < 0) {
        ...
        if (prop.recentCarbs) {
          info.unshift({
            ...
            , value: prop.recentCarbs.carbs + 'g @ ' + new Date(prop.recentCarbs.mills)
