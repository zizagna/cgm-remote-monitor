...
function init (ctx) {
  ...
  function buildTitle (prop, sbx) {
    var rangeLabel = prop.eventName ? sbx.translate(prop.eventName, { ci: true })
      .toUpperCase() : sbx.translate('Check BG');
  ...
  ar2.checkNotifications = function checkNotifications (sbx) {
    if (sbx.time - sbx.lastSGVMills() > times.mins(10)
      .msecs) {
      return;
    }
  ...
  function alexaAr2Handler (next, slots, sbx) {
    if (sbx.properties.ar2.forecast.predicted) {
      ...
      var response = 'You are expected to be between ' + min + ' and ' + max + ' over the ' + moment(maxForecastMills)
...
function incrementAR2 (result) {
  return {
    forecastTime: result.forecastTime + times.mins(5)
      .msecs
...
function buildDebug (prop, sbx) {
  return prop.forecast && {
    forecast: {
      ...
      , predicted: _.map(prop.forecast.predicted, function(p) { return sbx.scaleEntry(p) })
        .join(', ')
    }
  };
}
