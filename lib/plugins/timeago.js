...
function init (ctx) {
  ...
  timeago.checkStatus = function checkStatus (sbx) {
    ...
    function isStale (mins) {
      return sbx.time - lastSGVEntry.mills > times.mins(mins)
        .msecs;
    }
  ...
  timeago.inTheFuture = function inTheFuture (opts) {
    if (opts.entry.mills - times.mins(5)
      .msecs > opts.time) {
      return {
  ...
  timeago.resolvers = [
    timeago.isMissing, timeago.inTheFuture, timeago.almostInTheFuture, timeago.isLessThan(times.mins(2)
      .msecs, times.min()
      .msecs, 'min ago', 'm'), timeago.isLessThan(times.hour()
      .msecs, times.min()
      .msecs, 'mins ago', 'm'), timeago.isLessThan(times.hours(2)
      .msecs, times.hour()
      .msecs, 'hour ago', 'h'), timeago.isLessThan(times.day()
      .msecs, times.hour()
      .msecs, 'hours ago', 'h'), timeago.isLessThan(times.days(2)
      .msecs, times.day()
      .msecs, 'day ago', 'd'), timeago.isLessThan(times.week()
      .msecs, times.day()
      .msecs, 'days ago', 'd')
