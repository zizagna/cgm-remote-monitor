...
function init (ctx) {
  ...
  sensorState.getStateString = function findLatestState (sbx) {
    ...
    if (sensorInfo && sensorInfo.xdripjs) {
      ...
      if (sensorInfo.xdripjs.state != 0x6) {
        ...
        if (!lastStateNotification || (lastStateNotification.state != sensorInfo.xdripjs.state) || !prefs.stateNotifyIntrvl || (moment()
            .diff(lastStateNotification.timestamp, 'minutes') > (prefs.stateNotifyIntrvl * 60))) {
  ...
  sensorState.updateVisualisation = function updateVisualisation (sbx) {
    ...
    info.push({
      label: 'State Time: '
      , value: (sensor && sensor.lastStateTime && moment()
        .diff(sensor.lastStateTime, 'minutes') + ' minutes ago') || 'Unknown'
    });
    ...
    if (sensor && sensor.lastSessionStart && (sensor.lastState != 0x1)) {
      var diffTime = moment()
        .diff(moment(sensor.lastSessionStart));
    ...
    if (sensor) {
      if (sensor.lastTxActivation) {
        info.push({
          label: 'Tx Age: '
          , value: moment()
            .diff(moment(sensor.lastTxActivation), 'days') + ' days'
        });
      }
      ...
      if (sensor.lastCalibrationDate) {
        info.push({
          label: 'Calibration: '
          , value: moment()
            .diff(moment(sensor.lastCalibrationDate), 'hours') + ' hours ago'
        });
      }
      ...
      if (sensor.lastBatteryTimestamp) {
        info.push({
          label: 'Battery: '
          , value: moment()
            .diff(moment(sensor.lastBatteryTimestamp), 'minutes') + ' minutes ago'
        });
      }
