...
function init (ctx) {
  ...
  sage.getPrefs = function getPrefs (sbx) {
    return {
      info: sbx.extendedSettings.info || times.days(6)
        .hours
      , warn: sbx.extendedSettings.warn || (times.days(7)
        .hours - 4)
      , urgent: sbx.extendedSettings.urgent || (times.days(7)
        .hours - 2)
    ...
    ['Sensor Change', 'Sensor Start'].forEach(function eachEvent (event) {
      if (latest[event].found) {
        ...
        info.push({
          ...
          , value: new Date(latest[event].treatmentDate)
            .toLocaleString()
        });
