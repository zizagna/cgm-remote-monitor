...
function init (ctx) {
  ...
  cob.RECENCY_THRESHOLD = times.mins(30)
    .msecs;
  ...
  cob.lastCOBDeviceStatus = function lastCOBDeviceStatus (devicestatus, time) {
    ...
    var futureMills = time + times.mins(5)
      .msecs; //allow for clocks to be a little off
  ...
  cob.fromTreatments = function fromTreatments (treatments, devicestatus, profile, time, spec_profile) {
    ...
    _.each(treatments, function eachTreatment (treatment) {
      if (treatment.carbs && treatment.mills < time) {
        ...
        if (decaysin_hr > -10) {
          // units: BG
          var actStart = iob.calcTotal(treatments, devicestatus, profile, lastDecayedBy, spec_profile)
            .activity;
          var actEnd = iob.calcTotal(treatments, devicestatus, profile, cCalc.decayedBy, spec_profile)
            .activity;
  ...
  cob.updateVisualisation = function updateVisualisation (sbx) {
    ...
    if (lastCarbs) {
      var when = new Date(lastCarbs.mills)
        .toLocaleString();
