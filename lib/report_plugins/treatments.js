...
treatments.report = function report_treatments (datastorage, sorteddaystoshow, options) {
  ...
  function editTreatment (event) {
    ...
    $('#rped_eventType')
      .append('<option value="Temp Basal">' + translate('Temp Basal') + '</option>');
    $('#rped_eventType')
      .append('<option value="Bolus Wizard">' + translate('Bolus Wizard') + '</option>');
    ...
    client.profilefunctions.listBasalProfiles()
      .forEach(function(p) {
        $('#rped_profile')
          .append('<option id="' + p + '">' + p + '</option>');
      });

    $('#rp_edittreatmentdialog')
      .dialog({
        width: 360
        , height: 600
        , buttons: [
          {
            ...
            , class: 'leftButton'
            , click: function() {
              data.eventType = $('#rped_eventType')
                .val();
              data.glucose = $('#rped_glucoseValue')
                .val();
              data.glucoseType = $('#rp_edittreatmentdialog')
                .find('input[name=rp_bginput]:checked')
                .val();
              data.carbs = $('#rped_carbsGiven')
                .val();
              data.protein = $('#rped_proteinGiven')
                .val();
              data.fat = $('#rped_fatGiven')
                .val();
              data.insulin = $('#rped_insulinGiven')
                .val();
              data.duration = $('#rped_duration')
                .val();
              data.percent = $('#rped_percent')
                .val();
              data.absolute = $('#rped_absolute')
                .val();
              data.profile = $('#rped_profile')
                .val();
              data.notes = $('#rped_adnotes')
                .val();
              data.enteredBy = $('#rped_enteredBy')
                .val();
              data.eventTime = new Date(client.utils.mergeInputTime($('#rped_eventTimeValue')
                  .val(), $('#rped_eventDateValue')
                  .val()))
                .toISOString();
        ...
        , open: function() {
          ...
          $('#rped_bgfromsensor')
            .prop('checked', data.glucoseType === 'Sensor');
          $('#rped_bgfrommeter')
            .prop('checked', data.glucoseType === 'Finger');
          $('#rped_bgmanual')
            .prop('checked', data.glucoseType === 'Manual');
          $('#rped_carbsGiven')
            .val(data.carbs ? data.carbs : '');
          $('#rped_proteinGiven')
            .val(data.protein ? data.protein : '');
          $('#rped_fatGiven')
            .val(data.fat ? data.fat : '');
          $('#rped_insulinGiven')
            .val(data.insulin ? data.insulin : '');
          $('#rped_duration')
            .val(data.duration ? data.duration : '');
          $('#rped_percent')
            .val(data.percent ? data.percent : '');
          $('#rped_absolute')
            .val('absolute' in data ? data.absolute : '');
          $('#rped_profile')
            .val(data.profile ? data.profile : '');
          $('#rped_adnotes')
            .val(data.notes ? data.notes : '');
          $('#rped_enteredBy')
            .val(data.enteredBy ? data.enteredBy : '');
          $('#rped_eventTimeValue')
            .val(moment(data.created_at)
              .format('HH:mm'));
          $('#rped_eventDateValue')
            .val(moment(data.created_at)
              .format('YYYY-MM-DD'));
  ...
  sorteddaystoshow.forEach(function(day) {
    ...
    for (var t = 0; t < treatments.length; t++) {
      ...
      table.append($('<tr>')
        ...
        .append($('<td>')
          .append(new Date(tr.created_at)
            .toLocaleTimeString()
            .replace(/([\d]+:[\d]{2})(:[\d]{2})(.*)/, '$1$3')))
        .append($('<td>')
          .append(tr.eventType ? translate(client.careportal.resolveEventName(tr.eventType)) : ''))
        .append($('<td>')
          .attr('align', 'center')
          .append(tr.glucose ? tr.glucose + ' (' + translate(tr.glucoseType) + ')' : ''))
        .append($('<td>')
          .attr('align', 'center')
          .append(tr.insulin ? tr.insulin.toFixed(2) : ''))
        .append($('<td>')
          .attr('align', 'center')
          .append(tr.carbs ? tr.carbs : ''))
        .append($('<td>')
          .attr('align', 'center')
        ...
        .append($('<td>')
          .append(tr.enteredBy ? tr.enteredBy : ''))
        .append($('<td>')
          .append(tr.notes ? tr.notes : ''))
      );
    }
  });
  $('#treatments-report')
