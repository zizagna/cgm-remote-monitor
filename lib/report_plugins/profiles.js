...
profiles.report = function report_profiles (datastorage, sorteddaystoshow, options) {
  ...
  function recordChange (event) {
    if ($('#profiles-databaserecords option')
      .length < 1)
      return;
    ...
    $('#profiles-default')
      .val(currentrecord.defaultProfile);

    Object.keys(currentrecord.store).forEach(function(key) {
  ...
  function displayRecord (record, name) {
    ...
    table.append($('<tr>')
      .append($('<td>')
        .append('<b>' + translate('Units') + '</b>:&nbsp' + record.units)));
    table.append($('<tr>')
      .append($('<td>')
        .append('<b>' + translate('DIA') + '</b>:&nbsp' + record.dia)));
    table.append($('<tr>')
      .append($('<td>')
        .append('<b>' + translate('Timezone') + '</b>:&nbsp' + record.timezone)));
    table.append($('<tr>')
      .append($('<td>')
        .append('<b>' + translate('Carbs activity / absorption rate') + '</b>:&nbsp' + record.carbs_hr)));
    table.append($('<tr>')
      .append($('<td>')
        .append('<b>' + translate('Insulin to carb ratio (I:C)') + '</b>:&nbsp' + '<br>' + displayRanges(record.carbratio))));
    table.append($('<tr>')
      .append($('<td>')
        .append('<b>' + translate('Insulin Sensitivity Factor (ISF)') + '</b>:&nbsp' + '<br>' + displayRanges(record.sens))));
    table.append($('<tr>')
      .append($('<td>')
        .append('<b>' + translate('Basal rates [unit/hour]') + '</b>:&nbsp' + '<br>' + displayRanges(record.basal))));
    table.append($('<tr>')
      .append($('<td>')
        .append('<b>' + translate('Target BG range [mg/dL,mmol/L]') + '</b>:&nbsp' + '<br>' + displayRanges(record.target_low, record.target_high))));
