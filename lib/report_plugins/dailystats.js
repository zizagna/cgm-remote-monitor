...
dailystats.report = function report_dailystats (datastorage, sorteddaystoshow, options) {
  ...
  sorteddaystoshow.forEach(function(day) {
    ...
    if (daysRecords.length === 0) {
      $('<td/>')
        .appendTo(tr);
      $('<td class=\"tdborder\" style=\"width:160px\">' + report_plugins.utils.localeDate(day) + '</td>')
        .appendTo(tr);
    ...
    $('<td><div id=\"dailystat-chart-' + day.toString() + '\" class=\"inlinepiechart\"></div></td>')
      .appendTo(tr);

    $('<td class=\"tdborder\" style=\"width:160px\">' + report_plugins.utils.localeDate(day) + '</td>')
      .appendTo(tr);
    $('<td class=\"tdborder\">' + Math.round((100 * stats.lows) / daysRecords.length) + '%</td>')
      .appendTo(tr);
    $('<td class=\"tdborder\">' + Math.round((100 * stats.normal) / daysRecords.length) + '%</td>')
      .appendTo(tr);
    $('<td class=\"tdborder\">' + Math.round((100 * stats.highs) / daysRecords.length) + '%</td>')
      .appendTo(tr);
    $('<td class=\"tdborder\">' + daysRecords.length + '</td>')
      .appendTo(tr);
    $('<td class=\"tdborder\">' + minForDay + '</td>')
      .appendTo(tr);
    $('<td class=\"tdborder\">' + maxForDay + '</td>')
      .appendTo(tr);
    $('<td class=\"tdborder\">' + average.toFixed(1) + '</td>')
      .appendTo(tr);
    $('<td class=\"tdborder\">' + ss.standard_deviation(bgValues)
        .toFixed(1) + '</td>')
      .appendTo(tr);
    $('<td class=\"tdborder\">' + ss.quantile(bgValues, 0.25)
        .toFixed(1) + '</td>')
      .appendTo(tr);
    $('<td class=\"tdborder\">' + ss.quantile(bgValues, 0.5)
        .toFixed(1) + '</td>')
      .appendTo(tr);
    $('<td class=\"tdborder\">' + ss.quantile(bgValues, 0.75)
        .toFixed(1) + '</td>')
      .appendTo(tr);
