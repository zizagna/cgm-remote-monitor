'use strict';
var translate = require('../language')()
  .translate;
...
calibrations.report = function report_calibrations (datastorage, sorteddaystoshow) {
  ...
  var events = treatments.concat(cals)
    .concat(mbgs)
  ...
  function drawChart () {
    ...
    $('#calibrations-chart')
      .empty();
    var charts = d3.select('#calibrations-chart')
      .append('svg');
  ...
  function calcmbg (mbg) {
    ...
    if (lastsgv) {
      if (mbg.mills - lastsgv.mills > 5 * 60 * 1000) {
        console.log('Last SGV too old for MBG. Time diff: ' + ((mbg.mills - lastsgv.mills) / 1000 / 60)
          .toFixed(1) + ' min', mbg);
      } else {
