  ...
  daysToShow.forEach(function(day) {
    ...
    if (iobStatusAvailable) {
      ...
      var iobArray = client.plugins('iob')
        .IOBDeviceStatusesInTimeRange(datastorage.devicestatus, dayStart.valueOf(), dayEnd.valueOf());
      if (laDebug) console.log('getIOBs iobArray', iobArray);
      iobArray.forEach(function(entry) {
        var index = Math.floor(moment(entry.mills)
          .diff(dayStart, 'minutes') / 5);
  ...
  daysToShow.forEach(function(day) {
    ...
    if (cobStatusAvailable) {
      ...
      var cobArray = client.plugins('cob')
        .COBDeviceStatusesInTimeRange(datastorage.devicestatus, dayStart.valueOf(), dayEnd.valueOf());
...
loopalyzer.fillNanWithTreatments = function(array, treatments) {
  treatments.forEach(function(treatment) {
    var dayStart = moment(treatment.date)
      .startOf('day');
    var minutesAfterMidnight = moment(treatment.date)
...
loopalyzer.getCarbTreatments = function(datastorage, daysToShow) {
  ...
  var endDate = moment(daysToShow[daysToShow.length - 1])
    .add(1, 'days');

  datastorage.treatments.filter(function(treatment) { return treatment.carbs && treatment.carbs > 0 })
    .forEach(function(treatment) {
      if (moment(treatment.created_at)
...
loopalyzer.getInsulinTreatments = function(datastorage, daysToShow) {
  ...
  datastorage.treatments.filter(function(treatment) { return treatment.insulin && treatment.insulin > 0 })
    .forEach(function(treatment) {
      if (moment(treatment.created_at)
        .isBetween(startDate, endDate)) {
    ...
    if (predictions.length > 0 && treatmentTimestamps.length > 0) {
      ...
      for (var treatmentsIndex = 0; treatmentsIndex < treatmentTimestamps.length; treatmentsIndex++) {
        ...
        if (predictedIndex != null) {
          ...
          var end = moment(day)
            .endOf('day'); // Default to stop and end of the day
          ...
          for (var entryIndex in entry.values) {
            if (!d.isAfter(end)) {
              var dayStart = moment(d)
                .startOf('day');
              var minutesAfterMidnight = moment(d)
                .diff(dayStart, 'minutes');
              var index = Math.floor(minutesAfterMidnight / 5);
...
loopalyzer.renderProfilesTable = function(datastoreProfiles, daysToShow, client) {
    ...
    if (store) {
      for (var key in store) {
        ...
        if (store.hasOwnProperty(key)) {
          ...
          if ((newEntry.basal || newEntry.carbratio || newEntry.sens) && moment(entry.startDate)
            .isBefore(endOfLastDay))
  ...
  profilesArray2.forEach(function(entry) {
    if (laDebug)
      console.log(entry.startDate + ' isBefore ' + beginningOfFirstDay + ' = ' + moment(entry.startDate)
        .isBefore(beginningOfFirstDay));
    if (moment(entry.startDate)
      .isBefore(beginningOfFirstDay))
      latestProfile = entry;
  });
  ...
  profilesArray2.forEach(function(entry) {
    if (laDebug)
      console.log(entry.startDate + ' isAfter ' + beginningOfFirstDay + ' = ' + moment(entry.startDate)
  ...
  profiles.forEach(function(theProfile, index) {
    ...
    if (index < 3) {
      tableHtml += '<td><table>';
      tableHtml += '<caption>' + theProfile.profileName + ' (' + new Date(theProfile.startDate)
      ...
      if (theProfile.basal) {
        theProfile.basal.forEach(function(entry) {
          tableHtml += '<tr><td>' + entry.time + '</td><td>' + parseFloat(entry.value)
            .toFixed(3) + '</td></tr>'
        });
      }
      tableHtml += '</table></td>';
...
loopalyzer.generateReport = function(datastorage, daysToShow, options) {
  ...
  var dateInfo = moment(daysToShow[0])
    .format('ddd MMM D'); // .split(',')[0];
  if (daysToShow.length > 1) dateInfo += ' - ' + moment(daysToShow[daysToShow.length - 1])
    .format('ddd MMM D'); // .split(',')[0];
  $("#loopalyzer-dateinfo")
    .html(dateInfo);
  ...
  if (daysToShow.length == 1) {
    ...
    $("#rp_loopalyzertimeshift")
      .prop('checked', false);
    $("#rp_loopalyzertimeshift")
      .attr("disabled", true);
    $("#rp_loopalyzermincarbs")
      .attr("disabled", true);
    $("#rp_loopalyzert1")
      .attr("disabled", true);
    $("#rp_loopalyzert2")
      .attr("disabled", true);
    $("#rp_loopalyzertimeshiftinput")
      .css('color', 'gray');
  } else {
  ...
  if ($("#rp_loopalyzerprofiles")
    .is(":checked") && (datastorage.profiles && datastorage.profiles.length > 0)) {
    $("#loopalyzer-profiles-table")
      .show();
  ...
  daysToShow.forEach(function() {
    timeShifts.push(0);
    firstCarbs.push(NaN)
  });

  // Check to see if we are doing timeShift or not
  if ($("#rp_loopalyzertimeshift")
    .is(":checked") && daysToShow.length > 1) {
    var mealMinCarbs = $("#rp_loopalyzermincarbs")
      .val();
    var t1 = $("#rp_loopalyzert1")
      .val();
    var t2 = $("#rp_loopalyzert2")
      .val();
    ...
    if (t2 > t1) {
      ...
      var averageMinutesAfterMidnight = 0
        , sum = 0
        , count = 0;
