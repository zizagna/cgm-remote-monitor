...
daytoday.prepareHtml = function daytodayPrepareHtml (sorteddaystoshow) {
  ...
  sorteddaystoshow.forEach(function eachDay (d) {
    $('#daytodaycharts')
      .append($('<table><tr><td><div id="daytodaychart-' + d + '"></div></td><td><div id="daytodaystatchart-' + d + '"></td></tr></table>'));
  });
};
...
daytoday.report = function report_daytoday (datastorage, sorteddaystoshow, options) {
  ...
  if (options.insulindistribution)
    $('#daytodaycharts')
    .append('<br><br><b>' + translate('TDD average') + ':</b> ' + tddAverage.toFixed(1) + 'U <b>' +
  ...
  function drawChart (day, data, options) {
    ...
    charts = d3.select('#daytodaychart-' + day)
      .html(
        '<b>' +
    ...
    var cobStatusAvailable = client.plugins('cob')
      .isDeviceStatusAvailable(datastorage.devicestatus);
    var iobStatusAvailable = client.plugins('iob')
      .isDeviceStatusAvailable(datastorage.devicestatus);
    ...
    for (var dt = moment(from); dt < to; dt.add(5, 'minutes')) {
      if (options.iob && !iobStatusAvailable) {
        var iob = client.plugins('iob')
          .calcTotal(datastorage.treatments, datastorage.devicestatus, profile, dt.toDate())
          .iob;
      ...
      if (options.cob && !cobStatusAvailable) {
        var cob = client.plugins('cob')
          .cobTotal(datastorage.treatments, datastorage.devicestatus, profile, dt.toDate())
          .cob;
        if (!dt.isSame(from)) {
          cobpolyline += ', ';
        }
    ...
    if (cobStatusAvailable) {
      var lastdate = 0;
      var previousdate = 0;
      var cobArray = client.plugins('cob')
        .COBDeviceStatusesInTimeRange(datastorage.devicestatus, from.valueOf(), to.valueOf());
      _.each(cobArray, function drawCob (point) {
        if (previousdate !== 0 && point.mills - previousdate > times.mins(15)
          .msecs) {
          cobpolyline += ', ';
    ...
    if (iobStatusAvailable) {
      var lastdate = 0;
      var previousdate = 0;
      var iobArray = client.plugins('iob')
        .IOBDeviceStatusesInTimeRange(datastorage.devicestatus, from.valueOf(), to.valueOf());
    ...
    if (options.basal) {
      ...
      datastorage.tempbasalTreatments.forEach(function(t) {
        ...
        if (t.mills < to.format('x') && t.mills + times.mins(t.duration)
          .msecs > from.format('x')) {
          var text = g.append('text')
            ...
            .attr('dy', '.35em')
            .attr('x', xScale2((Math.max(t.mills, from.format('x')) + Math.min(t.mills + times.mins(t.duration)
              .msecs, to.format('x'))) / 2) + padding.left)
          ...
          if (text.node()
            .getBBox()
            .width > xScale2(t.mills + times.mins(t.duration)
              .msecs) - xScale2(t.mills)) {
      ...
      if (treatment.insulin || treatment.carbs || treatment.eventType == 'Temp Basal' || treatment.eventType == 'Combo Bolus') {
        ...
        if (treatment.duration && !treatment.cuttedby) {
          appendProfileSwitch(context, {
            ...
            , profile: client.profilefunctions.activeProfileToTime(times.mins(treatment.duration)
              .msecs + treatment.mills + 1)
            , mills: times.mins(treatment.duration)
              .msecs + treatment.mills
            , end: true
          });
        }
        ...
        context.append('rect')
          ...
          .attr('width', xScale2(treatment.mills + times.mins(treatment.duration)
            .msecs) - xScale2(treatment.mills))
        ...
        context.append('text')
          ...
          .attr('x', xScale2(treatment.mills + times.mins(treatment.duration)
            .msecs / 2) + padding.left)
    ...
    if (options.insulindistribution) {
      ...
      var table = $('<table>');
      $('<tr><td><b>' + translate('Bolus insulin:') + '</b></td><td align="right">' + bolusInsulin.toFixed(1) + 'U</td></tr>')
        .appendTo(table);
      if (positiveTemps > 0 || negativeTemps > 0) {
        $('<tr><td>' + translate('Base basal insulin:') + '</td><td align="right">' + baseBasalInsulin.toFixed(1) + 'U</td></tr>')
          .appendTo(table);
      }
      if (positiveTemps > 0) {
        $('<tr><td>' + translate('Positive temp basal insulin:') + '</td><td align="right">' + positiveTemps.toFixed(1) + 'U</td></tr>')
          .appendTo(table);
      }
      if (negativeTemps < 0) {
        $('<tr><td>' + translate('Negative temp basal insulin:') + '</td><td align="right">' + negativeTemps.toFixed(1) + 'U</td></tr>')
          .appendTo(table);
      }
      var totalBasalInsulin = baseBasalInsulin + positiveTemps + negativeTemps;
      $('<tr><td><b>' + translate('Total basal insulin:') + '</b></td><td align="right">' + totalBasalInsulin.toFixed(1) + 'U</td></tr>')
        .appendTo(table);
      var totalDailyInsulin = bolusInsulin + baseBasalInsulin + positiveTemps + negativeTemps;
      $('<tr><td><b>' + translate('Total daily insulin:') + '</b></td><td align="right">' + totalDailyInsulin.toFixed(1) + 'U</td></tr>')
        .appendTo(table);

      $('<tr><td>' + translate('Total carbs') + ':</td><td align="right">' + data.dailyCarbs + ' g</td></tr>')
        .appendTo(table);
      $('<tr><td>' + translate('Total protein') + ':</td><td align="right">' + data.dailyProtein + ' g</td></tr>')
        .appendTo(table);
      $('<tr><td>' + translate('Total fat') + ':</td><td align="right">' + data.dailyFat + ' g</td></tr>')
        .appendTo(table);

      $('<tr><td colspan="2"><span id="daytodaystatinsulinpiechart-' + day + '"></span><span id="daytodaystatcarbspiechart-' + day + '"></span></td></tr>')
        .appendTo(table);
      $('#daytodaystatchart-' + day)
        .append(table);
      ...
      var chartData = [
        {
          ...
          , pct: (totalBasalInsulin / totalDailyInsulin * 100)
            .toFixed(0)
        }
        ...
        , {
          ...
          , count: bolusInsulin
          , pct: (bolusInsulin / totalDailyInsulin * 100)
            .toFixed(0)
        }
      ];
      ...
      var carbscolor = d3.scale.ordinal()
        .range(['red']);
    ...
    console.log("Rendering " + day, new Date()
      .getTime() - timestart.getTime(), "msecs");
  }
