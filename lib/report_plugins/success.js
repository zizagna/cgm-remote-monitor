...
success.report = function report_success (datastorage, sorteddaystoshow, options) {
  ...
  var firstDataPoint = data.reduce(function(min, record) { return Math.min(min, record.displayTime); }, Number.MAX_VALUE);
  ...
  quarters = dim(quarters)
    ...
    .map(function(quarter, ix, all) {
      ...
      quarter.numberLow = bgValues.filter(function(bg) { return bg < low; }).length;
      quarter.numberHigh = bgValues.filter(function(bg) { return bg >= high; }).length;
