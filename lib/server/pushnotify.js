...
function init (env, ctx) {
  ...
  var receipts = new NodeCache({
    stdTTL: times.hour()
      .secs
    , checkperiod: times.mins(5)
      .secs
  });
  var recentlySent = new NodeCache({
    stdTTL: times.mins(15)
      .secs
    , checkperiod: 20
  });
  ...
  pushnotify.emitNotification = function emitNotification (notify) {
    ...
    console.log('Sending notify:', notify);
  ...
  pushnotify.pushoverAck = function pushoverAck (response) {
    ...
    if (notify) {
      ...
      ctx.notifications.ack(notify.level, notify.group, times.mins(snoozeMins)
        .msecs, true);
    ...
    if (ctx.pushover) {
      ...
      ctx.pushover.send(notify, function pushoverCallback (err, result) {
        if (err) {
          ...
          recentlySent.ttl(notify.key, times.mins(15)
            .secs);
