...
function storage (env, ctx) {
  ...
  function upsert (obj, fn) {
    ...
    api()
      .update(query, obj, { upsert: true }, function complete (err, updateResults) {
        if (!err) {
        ...
        if (!err && obj.preBolus) {
          ...
          var pbTreat = {
            created_at: (new Date(new Date(results.created_at)
                .getTime() + (obj.preBolus * 60000)))
              .toISOString()
            , eventType: obj.eventType
          ...
          api()
            .update(query, pbTreat, { upsert: true }, function pbComplete (err) {
  ...
  function remove (opts, fn) {
    return api()
      .remove(query_for(opts), function(err, stat) {
