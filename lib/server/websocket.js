...
function init (env, ctx, server) {
  ...
  function listeners () {
    io.sockets.on('connection', function onConnection (socket) {
      ...
      socket.on('dbUpdate', function dbUpdate (data, callback) {
        ...
        ctx.store.collection(collection)
          .update({ '_id': id }, { $set: data.data });
      ...
      socket.on('dbUpdateUnset', function dbUpdateUnset (data, callback) {
        ...
        ctx.store.collection(collection)
          .update({ '_id': objId }, { $unset: data.data });
      ...
      socket.on('dbAdd', function dbAdd (data, callback) {
        ...
        if (data.collection === 'treatments') {
          ...
          ctx.store.collection(collection)
            .find(query)
            .toArray(function findResult (err, array) {
              ...
              var query_similiar = {
                created_at: {
                  $gte: new Date(new Date(data.data.created_at)
                      .getTime() - maxtimediff)
                    .toISOString()
                  , $lte: new Date(new Date(data.data.created_at)
                      .getTime() + maxtimediff)
                    .toISOString()
                }
              };
              ...
              ctx.store.collection(collection)
                .find(query_similiar)
                .toArray(function findSimiliarResult (err, array) {
                  ...
                  if (err || array.length > 0) {
                    ...
                    ctx.store.collection(collection)
                      .update({ '_id': objId }, { $set: { created_at: data.data.created_at } });
                    if (callback) {
                      callback([array[0]]);
                    }
                    ctx.bus.emit('data-received');
                  ...
                  ctx.store.collection(collection)
                    .insert(data.data, function insertResult (err, doc) {
      ...
      socket.on('authorize', function authorize (message, callback) {
        verifyAuthorization(message, function verified (err, authorization) {
          ...
          if (socketAuthorization.read) {
            ...
            var msecHistory = times.hours(history)
              .msecs;
            // if `from` is received, it's a reconnection and full data is not needed
            if (from && from > 0) {
              filterTreatments = true;
              msecHistory = Math.min(new Date()
                .getTime() - from, msecHistory);
            }
            // send all data upon new connection
            if (lastData && lastData.splitRecent) {
              var split = lastData.splitRecent(Date.now(), times.hours(3)
                .msecs, msecHistory, filterTreatments);
              if (message.status) {
      ...
      socket.on('nsping', function ping (message, callback) {
        ...
        timeDiff = new Date()
          .getTime() - clientTime;
        console.log(LOG_WS + 'Ping from client ID: ', socket.client.id, ' client: ', clientType, ' timeDiff: ', (timeDiff / 1000)
          .toFixed(1) + 'sec');
        if (callback) {
          callback({
            result: 'pong'
  ...
  websocket.update = function update () {
    ...
    if (lastData.sgvs) {
      ...
      if (delta.delta) {
        console.log('lastData full size', JSON.stringify(lastData)
          .length, 'bytes');
        if (delta.sgvs) {
          console.log('patientData update size', JSON.stringify(delta)
            .length, 'bytes');
        }
        emitData(delta);
