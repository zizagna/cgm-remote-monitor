...
function init (env, callback) {
  ...
  var configPath = env.storageURI.split('openaps://')
    .pop();
  ...
  function loadData (path) {
    ...
    try {
      ...
      if (_.isArray(inputData)) {
        ...
        inputData.created_at = new Date(fs.statSync(path)
            .mtime)
          .toISOString();
        addId(inputData);
        inputData = [inputData];
  ...
  function reportAsCollection (name) {
    ...
    var wrapper = {
      ...
      , toArray: function toArray (callback) {
        collection.find(wrapper.findQuery)
          .toArray(function intercept (err, results) {
            ...
            if (wrapper.sortQuery) {
              var field = _.keys(wrapper.sortQuery)
                .pop();
