...
function init (profileData) {
  ...
  profile.loadData = function loadData (profileData) {
    if (profileData && profileData.length) {
      ...
      _.each(profile.data, function eachProfileRecord (record) {
        ...
        record.mills = new Date(record.startDate)
          .getTime();
      });
    }
  };
  ...
  profile.getValueByTime = function getValueByTime (time, valueType, spec_profile) {
    ...
    time = time + offset * times.hours(offset)
      .msecs;
    ...
    var t = profile.getTimezone(spec_profile) ? moment(minuteTime)
      .tz(profile.getTimezone(spec_profile)) : moment(minuteTime);

    // Convert to seconds from midnight
    var mmtMidnight = t.clone()
      .startOf('day');
    var timeAsSecondsFromMidnight = t.clone()
  ...
  profile.updateTreatments = function updateTreatments (profiletreatments, tempbasaltreatments, combobolustreatments) {
    ...
    _.each(profile.tempbasaltreatments, function addDuration (t) {
      t.endmills = t.mills + times.mins(t.duration || 0)
        .msecs;
    });
    ...
    profile.profiletreatments_hash = crypto.createHash('sha1')
      .update(JSON.stringify(profile.profiletreatments))
      .digest('hex');
    profile.tempbasaltreatments_hash = crypto.createHash('sha1')
      .update(JSON.stringify(profile.tempbasaltreatments))
      .digest('hex');
    profile.combobolustreatments_hash = crypto.createHash('sha1')
      .update(JSON.stringify(profile.combobolustreatments))
      .digest('hex');
  };
  ...
  profile.activeProfileTreatmentToTime = function activeProfileTreatmentToTime (time) {
    const cacheKey = 'profile' + time + profile.profiletreatments_hash;
    ...
    if (profile.hasData()) {
      profile.profiletreatments.forEach(function eachTreatment (t) {
        if (time >= t.mills && t.mills >= profile.data[0].mills) {
          const duration = times.mins(t.duration || 0)
            .msecs;
  ...
  profile.comboBolusTreatment = function comboBolusTreatment (time) {
    ...
    profile.combobolustreatments.forEach(function eachTreatment (t) {
      var duration = times.mins(t.duration || 0)
        .msecs;
  ...
  profile.listBasalProfiles = function listBasalProfiles () {
    ...
    if (profile.hasData()) {
      ...
      const store = profile.data[0].store;
      for (var key in store) {
        if (Object.prototype.hasOwnProperty.call(store, key) && key !== current) {
          if (key.indexOf('@@@@@') < 0)
            profiles.push(key);
        }
      }
