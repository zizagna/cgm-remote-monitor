...
function mergeToTreatments (ddata, results) {
  ...
  var treatments = _.map(filtered, function update (treatment) {
    treatment.mills = new Date(treatment.created_at)
      .getTime();
    return treatment;
  });

  //filter out temps older than a day and an hour ago since we don't display them
  var oldestAgo = ddata.lastUpdated - TWO_DAYS - times.hour()
    .msecs;
...
function loadActivity (ddata, ctx, callback) {
  var dateRange = {
    $gte: new Date(ddata.lastUpdated - (ONE_DAY * 2))
      .toISOString()
  };
  if (ddata.page && ddata.page.frame) {
    dateRange['$lte'] = new Date(ddata.lastUpdated)
      .toISOString();
  }
...
function loadTreatments (ddata, ctx, callback) {
  ...
  var dateRange = {
    $gte: new Date(ddata.lastUpdated - (ONE_DAY * 2.5))
      .toISOString()
  };
  if (ddata.page && ddata.page.frame) {
    dateRange['$lte'] = new Date(ddata.lastUpdated)
...
function loadProfileSwitchTreatments (ddata, ctx, callback) {
  var dateRange = {
    $gte: new Date(ddata.lastUpdated - (ONE_DAY * 31 * 12))
      .toISOString()
  };
  ...
  ctx.treatments.list(tq, function(err, results) {
    ...
    if (results) {
      ...
      for (var p = 0; p < results.length; p++) {
        var pdate = new Date(results[p].created_at)
          .getTime();
        if (pdate < now) {
...
function loadLatestSingle (ddata, ctx, dataType, callback) {
  ...
  var dateRange = {
    $gte: new Date(ddata.lastUpdated - (ONE_DAY * 32))
      .toISOString()
  };
...
function loadDeviceStatus (ddata, env, ctx, callback) {
  var dateRange = {
    $gte: new Date(ddata.lastUpdated - ONE_DAY)
      .toISOString()
  };
  if (ddata.page && ddata.page.frame) {
    dateRange['$lte'] = new Date(ddata.lastUpdated)
  ...
  ctx.devicestatus.list(opts, function(err, results) {
    if (!err && results) {
      ddata.devicestatus = _.map(results, function eachStatus (result) {
          result.mills = new Date(result.created_at)
            .getTime();
