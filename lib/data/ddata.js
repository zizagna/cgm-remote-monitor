...
function init () {
  ...
  ddata.splitRecent = function splitRecent (time, cutoff, max, treatmentsToo) {
    ...
    console.log('results.first size', JSON.stringify(result.first)
      .length, 'bytes');
    console.log('results.rest size', JSON.stringify(result.rest)
      .length, 'bytes');
  ...
  ddata.processDurations = function processDurations (treatments, keepzeroduration) {
    ...
    function cutIfInInterval (base, end) {
      if (base.mills < end.mills && base.mills + times.mins(base.duration)
        .msecs > end.mills) {
        base.duration = times.msecs(end.mills - base.mills)
          .mins;
        if (end.profile) {
