...
function configure (app, wares, ctx) {
  ...
  function ifModifiedSinceCTX (req, res, next) {
    ...
    if (!_isNil(lastEntry)) {
      lastEntryDate = new Date(_last(ctx.ddata.sgvs)
        .mills);
    ...
    if (lastEntryDate.getTime() <= new Date(ifModifiedSince)
      .getTime()) {
  ...
  function format_entries (req, res) {
    ...
    if (lastEntryDate !== null && ifModifiedSince !== null && lastEntryDate.getTime() <= new Date(ifModifiedSince)
      .getTime()) {
...
```
  ...
  function prep_patterns (req, res, next) {
    ...
    if (prefix.length === 1) {
      ...
      // an indexed field
      query[field].$regex = prefix.map(iter_regex('^'))
        .pop();
    }
