  ...
  plugins.createHTML = function createHTML (client) {
    ...
    plugins.eachPlugin(function addHtml (p) {
      ...
      for (var i = 0; i < p.actions.length; i++) {
        ...
        fs.append($('<i>')
          .append(translate(a.description)));
        fs.append($('<div>')
          .attr('id', 'admin_' + p.name + '_' + i + '_html'));
        fs.append($('<button>')
        ...
        fs.append($('<span>')
          .attr('id', 'admin_' + p.name + '_' + i + '_status'));
        if (a.init) {
          a.init(client);
        }
      }
