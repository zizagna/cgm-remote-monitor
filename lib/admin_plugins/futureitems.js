...
futureitems.actions[0].init = function init (client, callback) {
  ...
  function showOneTreatment (tr, table) {
    table.append($('<tr>')
      ...
      .append($('<td>')
        .attr('width', '10%')
        .append(valueOrEmpty(tr.enteredBy)))
      .append($('<td>')
        .attr('width', '20%')
        .append(valueOrEmpty(tr.notes)))
    );
  }
  ...
  $status.hide()
    .text(translate('Loading database ...'))
    .fadeIn('slow');
  var nowiso = new Date()
    .toISOString();
  $.ajax('/api/v1/treatments.json?&find[created_at][$gte]=' + nowiso, {
      ...
      , success: function(records) {
        ...
        $status.hide()
          .text(translate('Database contains %1 future records', { params: [records.length] }))
          .fadeIn('slow');
        var table = $('<table>')
          .css('margin-top', '10px');
        $('#admin_' + futureitems.name + '_0_html')
          .append(table);
      ...
      , error: function() {
        $status.hide()
          .text(translate('Error loading database'))
          .fadeIn('slow');
...
futureitems.actions[0].code = function deleteRecords (client, callback) {
  ...
  $status.hide()
    .text(translate('Deleting records ...'))
    .fadeIn('slow');
