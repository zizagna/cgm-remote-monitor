...
subjects.actions = [{
  ...
  , init: function init (client, callback) {
    ...
    $status.hide()
      .text(client.translate('Loading database ...'))
      .fadeIn('slow');
    var table = $('<table id="admin_subjects_table">')
      .css('margin-top', '10px');
    $('#admin_' + subjects.name + '_0_html')
      .append(table)
      .append(genDialog(client));
...
function reload (client, callback) {
  $.ajax({
    ...
    .done(function success (records) {
      ...
      $status.hide()
        .text(client.translate('Database contains %1 subjects', { params: [records.length] }))
        .fadeIn('slow');
    ...
    .fail(function fail (err) {
      $status.hide()
        .text(client.translate('Error loading database'))
        .fadeIn('slow');
      subjects.records = [];
      if (callback) {
        callback(err);
  ...
  $('#editsubjectdialog')
    .dialog({
      ...
      , open: function() {
        ...
        $(this)
          .parent()
          .find('button:contains("' + client.translate('Save') + '")')
          .css({ 'float': 'left' });
        $('#edsub_name')
          .val(subject.name || '')
          .focus();
        $('#edsub_roles')
          .val(subject.roles ? subject.roles.join(', ') : '');
        $('#edsub_notes')
          .val(subject.notes || '');
      }
...
function showSubject (subject, table, client) {
  ...
  table.append($('<tr>')
    ...
    .append($('<td>')
      .attr('width', '20%')
      .append(subject.roles ? subject.roles.join(', ') : '[none]'))
    .append($('<td>')
      .attr('width', '20%')
      .append('<a href="/?token=' + subject.accessToken + '" target="_blank">' + subject.accessToken + '</a>'))
    .append($('<td>')
      .attr('width', '10%')
      .append(subject.notes ? subject.notes : ''))
  );
}
  ...
  table.empty()
    .append($('<tr>')
      ...
      .append($('<th>')
        .css('width', '100px')
        .attr('align', 'left')
        .append(client.translate('Name')))
      .append($('<th>')
        .css('width', '150px')
        .attr('align', 'left')
        .append(client.translate('Roles')))
      .append($('<th>')
        .css('width', '150px')
        .attr('align', 'left')
        .append(client.translate('Access Token')))
      .append($('<th>')
        .css('width', '150px')
        .attr('align', 'left')
        .append(client.translate('Notes')))
    );
