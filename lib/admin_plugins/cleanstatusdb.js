...
cleanstatusdb.actions[0].init = function init (client, callback) {
  ...
  $.ajax('/api/v1/devicestatus.json?count=500', {
      ...
      , success: function(records) {
        ...
        $status.hide()
          .text(translate('Database contains %1 records', { params: [recs] }))
          .fadeIn('slow');
      }
      , error: function() {
        $status.hide()
...
cleanstatusdb.actions[0].code = function deleteRecords (client, callback) {
  ...
  $.ajax({
    ...
    .done(function success () {
      $status.hide()
        .text(translate('All records removed ...'))
        .fadeIn('slow');
      if (callback) {
        callback();
      }
...
cleanstatusdb.actions[1].init = function init (client, callback) {
  ...
  $('#admin_' + cleanstatusdb.name + '_1_html')
    .html(numDays);
...
cleanstatusdb.actions[1].code = function deleteOldRecords (client, callback) {
  var translate = client.translate;
  var $status = $('#admin_' + cleanstatusdb.name + '_1_status');
  var numDays = Number($('#admin_devicestatus_days')
    .val());

  if (isNaN(numDays) || (numDays < 1)) {
    alert(translate('%1 is not a valid number', {
      params: [$('#admin_devicestatus_days')
        .val()]
    }));
    if (callback) { callback(); }
    return;
