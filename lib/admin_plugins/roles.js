...
roles.actions = [{
  ...
  , init: function init (client, callback) {
    ...
    $status.hide()
      ...
      .fadeIn('slow');
    var table = $('<table id="admin_roles_table">')
      .css('margin-top', '10px');
    $('#admin_' + roles.name + '_0_html')
      .append(table)
...
function reload (client, callback) {
  $.ajax({
    ...
    .done(function success (records) {
      ...
      $status.hide()
        .text(client.translate('Database contains %1 roles', { params: [records.length] }))
        .fadeIn('slow');
    ...
    .dialog({
      ...
      , buttons: [
        {
          ...
          , click: function() {
            ...
            role.name = $('#edrole_name')
              .val();
            role.permissions =
              _.chain($('#edrole_permissions')
                .val()
                .toLowerCase()
                .split(/[;, ]/))
              .map(_.trim)
      ...
      , open: function() {
        ...
        $('#edrole_name')
          ...
          .focus();
        $('#edrole_permissions')
          .val(role.permissions ? role.permissions.join(' ') : '');
        $('#edrole_notes')
          .val(role.notes || '');
      }
...
function showRole (role, table, client) {
  ...
  table.append($('<tr>')
    ...
    .append($('<td>')
      .attr('width', '20%')
      .append(_.isEmpty(role.permissions) ? '[none]' : role.permissions.join(' ')))
    .append($('<td>')
      .attr('width', '10%')
      .append(role._id ? (role.notes ? role.notes : '') : '[system default]'))
  );
}
  ...
  table.empty()
    .append($('<tr>')
      ...
      .append($('<th>')
        .css('width', '150px')
        .attr('align', 'left')
        .append(client.translate('Permissions')))
      .append($('<th>')
        .css('width', '150px')
        .attr('align', 'left')
