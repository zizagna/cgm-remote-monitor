...
cleantreatmentsdb.actions[0].init = function init (client, callback) {
  ...
  $('#admin_' + cleantreatmentsdb.name + '_0_html')
    .html(numDays);
...
cleantreatmentsdb.actions[0].code = function deleteOldRecords (client, callback) {
  var translate = client.translate;
  var $status = $('#admin_' + cleantreatmentsdb.name + '_0_status');
  var numDays = Number($('#admin_treatments_days')
    .val());

  if (isNaN(numDays) || (numDays < 3)) {
    alert(translate('%1 is not a valid number - must be more than 2', {
      params: [$('#admin_treatments_days')
        .val()]
    }));
    if (callback) { callback(); }
    return;
  ...
  $.ajax('/api/v1/treatments/?find[created_at][$lte]=' + dateStr, {
      ...
      , success: function(retVal) {
        $status.hide()
          .text(translate('%1 records deleted', { params: [retVal.n] }))
          .fadeIn('slow');
      }
      , error: function() {
        $status.hide()
